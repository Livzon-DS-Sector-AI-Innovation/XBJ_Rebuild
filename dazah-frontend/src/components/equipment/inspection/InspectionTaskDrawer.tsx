...
export function InspectionTaskDrawer({ templates, equipments }: Props) {
  ...
  useEffect(() => {
    if (taskDrawerOpen) {
      ...
      fetchEmployees({ page_size: 999 }).then(r => setPersonnel(r.data)).catch(() => {})
    }
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <Form.Item name="assigned_to" noStyle rules={[{ required: true, message: '请选择巡检人员' }]}>
              <Select showSearch placeholder="选择巡检人员" optionFilterProp="label"
                popupMatchSelectWidth={false} style={{ width: '100%' }}
                options={personnel.map(p => ({ label: `${p.name}${p.department ? ' · ' + p.department : ''}`, value: p.id }))}
                optionRender={o => {
                  const p = personnel.find(pe => pe.id === o.value)
                  if (!p) return <Text>{o.label}</Text>
                  return (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <Avatar size={28} style={{ backgroundColor: avColor(p.name), flexShrink: 0, fontSize: 11, fontWeight: 700 }}
                        icon={<UserOutlined />}>
                        {p.name.charAt(0)}
                      </Avatar>
                      <div style={{ lineHeight: 1.3 }}>
                        <Text style={{ fontSize: 13, fontWeight: 500 }}>{p.name}</Text>
                        {p.department && <Text type="secondary" style={{ fontSize: 11, display: 'block' }}>{p.department}{p.employee_number ? ` · ${p.employee_number}` : ''}</Text>}
                      </div>
                    </div>
                  )
                }} />
